...
$GLOBALS [G_FLEA_VAR] = array ('APP_INF' => array (), 'OBJECTS' => array (), 'DBO' => array (), 'CLASS_PATH' => array (), 'FLEA_EXCEPTION_STACK' => array (), 'FLEA_EXCEPTION_HANDLER' => null );
 ...
 * 载入默认设置文件
 */
$GLOBALS [G_FLEA_VAR] ['APP_INF'] = require (FLEA_DIR . '/DEPLOY_MODE_CONFIG.php');
...
class FLEA {
	 ...
	 */
	function import($dir) {
		if (array_search ( $dir, $GLOBALS [G_FLEA_VAR] ['CLASS_PATH'], true )) {
			return;
		}
	 ...
	 * 载入指定的文件
	 *
	 * FLEA::loadFile() 会 $filename 参数中的 “_” 替换为目录
	 *
	...
	function loadFile($filename, $loadOnce = false) {
		...
		if ($path != '') {
			if (isset ( $is_loaded [$path] ) && $loadOnce) {
				return true;
			}
			$is_loaded [$path] = true;
			if ($loadOnce) {
	...
	function runMVC() {
		...
		// 载入调度器并转发请求到控制器
		$dispatcherClass = 'FLEA_Dispatcher';
...
function __FLEA_EXCEPTION_HANDLER($ex) {
	...
	include (FLEA_DIR . "/FLEA_EXCEPTION.php");
	exit ();
}
...
function _ET($errorCode, $appError = false) {
	...
	if (! isset ( $message [$language] )) {
		$message [$language] = include (FLEA_DIR . '/ErrorMessage.php');
	}
	return isset ( $message [$language] [$errorCode] ) ? $message [$language] [$errorCode] : '';
}
